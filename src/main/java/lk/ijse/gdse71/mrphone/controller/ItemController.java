...
import lk.ijse.gdse71.mrphone.model.*;
...
public class ItemController {
    ...
    void deleteItem(ActionEvent event) throws SQLException, ClassNotFoundException {
        ...
        if (optionalButtonType.isPresent() && optionalButtonType.get() == ButtonType.YES) {
            boolean isDeleted = itemModel.delete(item_id);
            if (isDeleted) {
                refreshPage();
...
//        }
        ...
        try {
            boolean isSavedI = ItemModel.save(itemDto);
...
//        }
        ...
        boolean isUpdateI = itemModel.update(itemDto);
        boolean isUpdateID = itemDetailModel.updateItem(itemDetailDto);
